...
bool pcmove::actual_move() {
  ...
  if(items[itOrbDomination] > ORBBASE && isMountable(c2->monst) && fmsMove) {
    ...
    invismove = (turncount >= noiseuntil) && (items[itOrbInvis] > 0 || turncount % 2);
    killFriendlyIvy();
...
bool pcmove::attack() {
  ...
  if(items[itCurseWeakness] || c2->monst == moGargoyle || c2->monst == moHexer || (isStunnable(c2->monst) && c2->hitpoints > 1)) {
